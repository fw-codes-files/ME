...
master_default_configuration.wired_sync_mode = pykinect.k4a._k4a.K4A_WIRED_SYNC_MODE_MASTER
master_default_configuration.color_resolution = pykinect.K4A_COLOR_RESOLUTION_720P
master_default_configuration.depth_mode = pykinect.k4a._k4a.K4A_DEPTH_MODE_NFOV_UNBINNED
master_default_configuration.synchronized_images_only = True
master_default_configuration.color_format = pykinect.k4a._k4a.K4A_IMAGE_FORMAT_COLOR_BGRA32
...
subordinate_default_configuration.wired_sync_mode = pykinect.k4a._k4a.K4A_WIRED_SYNC_MODE_SUBORDINATE
subordinate_default_configuration.color_resolution = pykinect.K4A_COLOR_RESOLUTION_720P
subordinate_default_configuration.depth_mode = pykinect.k4a._k4a.K4A_DEPTH_MODE_NFOV_UNBINNED
subordinate_default_configuration.synchronized_images_only = True
subordinate_default_configuration.color_format = pykinect.k4a._k4a.K4A_IMAGE_FORMAT_COLOR_BGRA32



def capture_producer(flag, device_id: int ,config: pykinect.Configuration, queue: Queue, capture_process: Callable = default_capture_process):
    """capture
    ...
    while flag.value != 2:
        ...
        timestamp = pykinect.k4a._k4a.k4a_image_get_timestamp_usec(color_image_object.handle())
        ret, data = capture_process(capture, color_image_object, timestamp)
        if not ret:
            continue
        queue.put(data)
...
def synchronous_producer(flag, lock: Lock, queue: Queue, capture_producer_queue_list: List[Queue], max_dist: float=10 * 1000):
    """同步
...
class MulDeviceSynCapture(object):
    def __init__(self, master_device_id: int = 0, subordinate_device_ids: Union[int, List[int]] = 1, master_config: pykinect.Configuration = master_default_configuration, subordinate_config: pykinect.Configuration = subordinate_default_configuration, synmaxsize: int = 10, capmaxsize: int = 10, max_dist: float = 1000 * 10, capture_process: Callable = default_capture_process) -> None:
        """多相机同步捕获
        ...
        Args:
            ...
            max_dist (float, optional): 同步帧阈值(单位微妙). Defaults to 1000 * 10.
        Examples:
        ...
        assert pykinect.Device.device_get_installed_count() >= len(self.subordinate_device_ids) + 1, "Not enough devices installed"
        ...
        self._flag = Value('i', 0)  # 用于传递关闭信号, 0: 启动, 1: 关闭同步进程, 2: 关闭capture进程
        self.process_list = []
        self._lock = Lock()
        self.start()
    ...
    def start(self):
        ...
        self.process_list = []
        # 同步进程
        process = Process(target=synchronous_producer, args=(self._flag, self._lock, self.synchronous_queue, [self._master_queue] + self._subordinate_queue_list, self.max_dist))
        process.start()
        self.process_list.append(process)
        
        # 创建master process
        process = Process(target=capture_producer, args=(self._flag, self.master_device_id, self.master_config, self._master_queue, self.capture_process))
        process.start()
        self.process_list.append(process)
        
        # 创建subordinate process
        for device_id, queue in zip(self.subordinate_device_ids, self._subordinate_queue_list):
            process = Process(target=capture_producer, args=(self._flag,  device_id, self.subordinate_config, queue, self.capture_process))
            process.start()
            self.process_list.append(process)
